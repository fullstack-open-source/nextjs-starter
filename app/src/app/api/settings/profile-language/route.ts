 ...
 * POST /api/settings/profile-language
 */
export async function POST(req: NextRequest) {
  try {
    const { user } = await validateRequest(req);
    const permissionError = await checkPermissionOrReturnError(user, 'update_language');
    ...
    if (!language || !['en', 'ar', 'ind', 'fr', 'es', 'de'].includes(language)) {
    ...
    return SUCCESS.json('Profile language update successfully', serializedData);
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    logger.error('Error updating profile language', {
      module: 'Profile',
      extraData: {
        error: errorMessage,
        label: 'UPDATE_LANGUAGE',
      },
    });
