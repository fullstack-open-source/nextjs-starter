...
/**
 * Get Users by Type
 * GET /api/dashboard/users-by-type
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const fetchUsersByTypeData = async () => {
      ...
      return statsResult.map((row: { auth_type: string | null; _count: { auth_type: number } }) => ({
        user_type: row.auth_type || 'unknown',
    ...
    const stats = forceRefresh
      ? await reCache(fetchUsersByTypeData, {
          key: getDashboardStatisticsCacheKey('users-by-type'),
          duration: 'long', // Cache for 1 hour
        })
      : await withCache(fetchUsersByTypeData, {
          key: getDashboardStatisticsCacheKey('users-by-type'),
          duration: 'long', // Cache for 1 hour
        });

    return SUCCESS.json('User statistics by type retrieved successfully', {
      users_by_type: stats,
    });
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    logger.error('Error in users by type', {
      module: 'Dashboard',
      extraData: {
        error: errorMessage,
        label: 'USERS_BY_TYPE',
